...
**Ans**: Zero-based indexing means the first element in an array is at position 0, not position 1. For example, in the array fruits = ["apple", "banana",             "orange"],"apple" is at index 0, "banana" at index 1, and "orange" at index 2.  **OR**  This means the counting of elements starts at 0 rather than          1. For example, in a list [10, 20, 30], the first element (10) is at index 0.
...
**5. Given the Python array(list) scores = [78, 82, 65, 90, 73], write a Python code to print the total number of scores in the array.**  
   **Ans**: print(len(scores))  
    **Reason**: The built-in **len()** function calculates and returns the number of elements in an object. The object must be a sequence or collection type     that has a defined length, such as a list, string, tuple, or dictionary. In this case, the output will be 5, which is the number of elements in the list.
